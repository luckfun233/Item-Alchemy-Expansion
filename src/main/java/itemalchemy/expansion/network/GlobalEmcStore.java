...
/**
 * 全局 EMC 存储：把 EMC 修改持久化到前置模组自带的 {@code config/itemalchemy/emc_config.json}，对所有存档生效。
 *
 * <p>玩家在 SetEmcScreen 选择「所有存档（全局）」时，EMC 修改写入全局配置，任意存档加载该配置时都会生效。
 * 复用前置模组 {@link EMCManager} 的 Config 对象与文件路径，保证与前置模组的加载逻辑一致：
 * 内存 map 已由 {@link SetEmcNetwork#applyOnServer} 调用 {@link EMCManager#set} 更新，
 * 本类只负责持久化到配置文件。</p>
 *
