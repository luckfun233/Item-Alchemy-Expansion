 ...
 *
 * <p>由于 {@code AlchemyTableScreenHandler} 是上游 Item Alchemy 的类，不能直接修改其字段。
 * 用 Mixin {@code implements} 本接口 + {@code @Unique} 字段实现，外部代码通过 cast 调用。</p>
 *
 * <p>客户端与服务端各持有自己的 {@code AlchemyTableScreenHandler} 实例，各自的 filterMode 独立。
 * 客户端切换时本地立即生效 + 通过 {@code filter_mode} 网络包同步服务端。</p>
 */
