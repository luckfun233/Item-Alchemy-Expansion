...
public class NoEmcShulkerBoxToast implements Toast {
    ...
    public Visibility draw(MatrixStack matrices, ToastManager manager, long currentTime) {
        ...
        DrawableHelper.drawTexture(matrices, 0, 0, 0, 0, getWidth(), getHeight(), 160, 32);

        MinecraftClient client = MinecraftClient.getInstance();

        Text title = Text.translatable("itemalchemy-expansion.shulker_box.toast.title");
        client.textRenderer.drawWithShadow(matrices, title, 30, 7, 0xFFFF5555);
        ...
        client.textRenderer.drawWithShadow(matrices, descStr, 30, 18, 0xFFFFFFFF);

        client.getItemRenderer().renderGuiItemIcon(noEmcItem, 8, 8);
