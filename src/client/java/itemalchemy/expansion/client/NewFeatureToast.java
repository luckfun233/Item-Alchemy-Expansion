...
public class NewFeatureToast implements Toast {
    ...
    public Visibility draw(MatrixStack matrices, ToastManager manager, long currentTime) {
        ...
        drawTexture(matrices, 0, 0, 0, 0, 160, 32);
        drawTexture(matrices, 160, 0, 160, 0, getWidth() - 160, 32);
        drawTexture(matrices, 0, 32, 0, 0, 160, getHeight() - 32);
        drawTexture(matrices, 160, 32, 160, 0, getWidth() - 160, getHeight() - 32);

        MinecraftClient client = MinecraftClient.getInstance();

        Text title = Text.translatable("itemalchemy-expansion.new_feature_toast.title");
        client.textRenderer.drawWithShadow(matrices, title, 30, 7, 0xFFFFAA00);

        Text desc1 = Text.translatable("itemalchemy-expansion.new_feature_toast.desc1");
        client.textRenderer.drawWithShadow(matrices, desc1, 30, 20, 0xFFFFFFFF);
        ...
        client.textRenderer.drawWithShadow(matrices, desc2Str, 30, 31, 0xFFA0A0A0);

        DrawableHelper.drawTexture(matrices, 8, 12, 0, 0, 16, 16, 16, 16);

        return elapsed >= DURATION_MS ? Visibility.HIDE : Visibility.SHOW;
    }

    /** 1.19.2 无 context.drawTexture(Identifier,...) 版本，改用 DrawableHelper 静态方法 */
    private static void drawTexture(MatrixStack matrices, int x, int y, int u, int v, int width, int height) {
        DrawableHelper.drawTexture(matrices, x, y, u, v, width, height, 160, 32);
    }
