...
public final class AlchemyTableScreenShulkerPreview {
     ...
     */
    public static void onAfterRender(AlchemyTableScreen screen, MatrixStack matrices, int mouseX, int mouseY, float delta) {
        IAExpConfig config;
        try {
        ...
        try {
            renderPreview(matrices, mouseX, mouseY, shulkerBox, searchCtx);
        } catch (Throwable t) {
     ...
     */
    private static void renderPreview(MatrixStack matrices, int mouseX, int mouseY, ItemStack shulkerBox,
        ...
        matrices.push();
        matrices.translate(0, 0, Z_LAYER);

        try {
            // 背景 high alpha 避免下方图标透过来
            fill(matrices, x, y, x + bgWidth, y + bgHeight, 0xF0101010);
            GuiRenderUtil.drawBorder(matrices, x, y, bgWidth, bgHeight, 0xFF505050);
            ...
            drawTextWithShadow(matrices, client.textRenderer, title, x + PADDING, y + PADDING, 0xFFFFFF);
            ...
            for (int row = 0; row < ROWS; row++) {
                for (int col = 0; col < COLS; col++) {
                    ...
                    fill(matrices, sx, sy, sx + SLOT_SIZE, sy + SLOT_SIZE, 0x80202020);
                    fill(matrices, sx, sy, sx + SLOT_SIZE, sy + 1, 0xFF303030);
                    fill(matrices, sx, sy + SLOT_SIZE - 1, sx + SLOT_SIZE, sy + SLOT_SIZE, 0xFF303030);
                    fill(matrices, sx, sy, sx + 1, sy + SLOT_SIZE, 0xFF303030);
                    fill(matrices, sx + SLOT_SIZE - 1, sy, sx + SLOT_SIZE, sy + SLOT_SIZE, 0xFF303030);

                    ItemStack item = contents[idx];
                    if (item.isEmpty()) continue;

                    drawItem(matrices, item, sx + 1, sy + 1);
                    ...
                    if (item.getCount() > 1) {
                        ...
                        drawTextWithShadow(matrices, client.textRenderer, countText,
                    ...
                    if (redFrameEnabled && (row != focusRow || col != focusCol)
                        ...
                        renderRedFrame(matrices, sx, sy);
                    }
                }
            }

            if (focusRow >= 0 && focusCol >= 0) {
                renderFocus(matrices, client, x, gridY, bgWidth, bgHeight, screenW, contents);
            }
        } finally {
            matrices.pop();
        }
     ...
     */
    private static void renderFocus(MatrixStack matrices, MinecraftClient client,
        ...
        // 2px 粗边框，外延 1px
        fill(matrices, fsx - 1, fsy - 1, fcx + 1, fsy, white);
        fill(matrices, fsx - 1, fcy, fcx + 1, fcy + 1, white);
        fill(matrices, fsx - 1, fsy, fsx, fcy, white);
        fill(matrices, fcx, fsy, fcx + 1, fcy, white);
        matrices.pop();
        ...
        fill(matrices, labelX, labelY, labelX + labelW, labelY + labelH, 0xF0101010);
        GuiRenderUtil.drawBorder(matrices, labelX, labelY, labelW, labelH, 0xFF505050);
        drawTextWithShadow(matrices, client.textRenderer, name, labelX + 4, labelY + 4, 0xFFFFFF);
        drawTextWithShadow(matrices, client.textRenderer, emcLine, labelX + 4, labelY + 14, 0xFFFFD700);
        matrices.pop();
    }
     ...
     */
    private static void renderRedFrame(MatrixStack matrices, int sx, int sy) {
        matrices.push();
        matrices.translate(0, 0, 260);
        int red = 0xFFFF3030;
        // 2px 粗边框，外延 1px
        fill(matrices, sx - 1, sy - 1, sx + SLOT_SIZE + 1, sy, red);                       // top
        fill(matrices, sx - 1, sy + SLOT_SIZE, sx + SLOT_SIZE + 1, sy + SLOT_SIZE + 1, red); // bottom
        fill(matrices, sx - 1, sy, sx, sy + SLOT_SIZE, red);                               // left
        fill(matrices, sx + SLOT_SIZE, sy, sx + SLOT_SIZE + 1, sy + SLOT_SIZE, red);       // right
matrices.pop();
    }

    /** 1.19.2 fill 封装（DrawableHelper.fill 是静态 MatrixStack 版本） */
    ...
    /** 1.19.2 drawTextWithShadow 封装 */
    private static void drawTextWithShadow(MatrixStack matrices, net.minecraft.client.font.TextRenderer tr, String text, int x, int y, int color) {
        net.minecraft.client.gui.DrawableHelper.drawTextWithShadow(matrices, tr, net.minecraft.text.Text.literal(text), x, y, color);
    }

    /** 1.19.2 drawTextWithShadow 封装（Text 版本） */
    private static void drawTextWithShadow(MatrixStack matrices, net.minecraft.client.font.TextRenderer tr, net.minecraft.text.Text text, int x, int y, int color) {
        net.minecraft.client.gui.DrawableHelper.drawTextWithShadow(matrices, tr, text, x, y, color);
    }

    /** 1.19.2 物品图标渲染（无 DrawContext，改走 RenderSystem 模型视图矩阵以支持 z/scale） */
    private static void drawItem(MatrixStack matrices, ItemStack stack, int x, int y) {
